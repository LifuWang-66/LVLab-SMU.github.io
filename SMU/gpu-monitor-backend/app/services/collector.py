...
def get_alert_candidates(db: Session) -> list[dict[str, str]]:
    now = datetime.utcnow().replace(tzinfo=None)
    required_samples = _required_samples_for_eight_hours()
    lifu_profile = db.scalar(select(UserProfile).where(UserProfile.username == 'lifu'))
    cc_email = (lifu_profile.email or '').strip() if lifu_profile and lifu_profile.email else ''

    candidates: list[dict[str, str]] = []
    hosts = db.scalars(select(Host).where(Host.enabled.is_(True))).all()
    for host in hosts:
        current_rows = db.scalars(select(CurrentGpuStatus).where(CurrentGpuStatus.host_id == host.id)).all()
        per_user_gpu_count: dict[str, int] = {}
        for row in current_rows:
            active_users = [name.strip() for name in (row.active_users or '').split(',') if name.strip()]
        ...
        for username, gpu_count in per_user_gpu_count.items():
            ...
            eight_hour_max, sample_count = _get_eight_hour_max_util(db, host.id, username, now)
            if sample_count < required_samples:
                continue
            if LOW_UTIL_THRESHOLD <= eight_hour_max <= MID_UTIL_THRESHOLD:
                candidates.append(
            ...
            elif 0 <= eight_hour_max < LOW_UTIL_THRESHOLD:
                candidates.append(
                    {
                        'host_name': host.name,
                        'host_address': host.address,
                        ...
                        'cc_email': cc_email,
                        'event_type': 'avg_util_8h_below_40_killed',
                        'reason': f'Your 8-hour max GPU utilization is {eight_hour_max:.2f}% (below 40%).',
                    }
                )
    return candidates
