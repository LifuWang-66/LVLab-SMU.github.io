...
- 默认数据库：SQLite，当前建议路径是 `/home/lifu/workspace/GPUMonitor/data/gpu_monitor.db`。
...
DATABASE_URL=sqlite:////home/lifu/workspace/GPUMonitor/data/gpu_monitor.db
...
cd ~/GPUMonitor
conda activate gpu-monitor
python scripts/backup_sqlite.py --database-url sqlite:////home/lifu/workspace/GPUMonitor/data/gpu_monitor.db --output ./backups/gpu_monitor_backup.db
```
...
可以部署成公网可访问服务，但**默认配置不建议直接暴露到公网**。当前代码层面可以被外部访问（例如你用 `uvicorn ... --host 0.0.0.0` 并放开防火墙后），但上线前请至少完成下面检查：

1. **入口安全**：建议放在 Nginx/Caddy 反向代理后，并启用 HTTPS。
2. **会话安全**：将 `.env` 中 `SECRET_KEY` 改为高强度随机值，不要使用示例值。
3. **账号输入风险**：该系统会接收用户 SSH 凭据，公网部署时必须加 TLS，并建议增加额外登录保护（如组织账号 SSO 或 IP 白名单）。
4. **CORS 与来源限制**：默认 CORS 允许较宽，公网场景建议改为仅允许你的正式域名。
5. **网络隔离**：只暴露 Web 端口，不要把 GPU 服务器 SSH 端口直接暴露到公网；采集应在内网完成。
6. **运维策略**：开启日志审计、失败重试告警、数据库定期备份。

如果只是让实验室成员外网访问，推荐方案：
- 公网只开放 `443`（HTTPS）给反向代理。
- FastAPI 保持内网监听（或仅对代理可达）。
- 使用 Cloudflare Tunnel / VPN / 校园网网关做额外访问控制。

## 跨服务器发信模式（新增）

如果 `10.193.104.165` 能看到完整 GPU 数据，但另一台 Windows 机器才有 SMTP 发送能力，可用如下架构：

1. 在本服务（`10.193.104.165`）配置 `INTERNAL_API_TOKEN`，开放内部接口：`/api/internal/alert-candidates`。
2. 在 Windows 机器运行 `SMU/email-relay-service`，定时拉取上述接口并从 Windows 本机发邮件。

这样可以把“告警判断”留在数据源服务器，把“邮件发送”放在可发信服务器上。
